// tb_ml_kem_vector_firmware.cpp
//
// Testbench de Verilator (Fase 5, medicion de speedup): corre
// test_ml_kem_vector_firmware.hex sobre core_top_pipelined.sv — la
// version ACELERADA del protocolo ML-KEM-512 completo, usando las 8
// instrucciones vectoriales de la Fase 4 (via sw/asm/poly_vector_ops.S)
// en vez de poly_ntt.c escalar. Debe producir EXACTAMENTE el mismo
// resultado criptografico que tb_ml_kem_firmware.cpp (version
// escalar) — el conteo de ciclos es la metrica de interes aca, no la
// correctitud en si (ya establecida por la version escalar).
...
int main(int argc, char** argv) {
    ...
    if (errors == 0) {
        std::printf("\nPASS: ML-KEM-512 ACELERADO (vectorial) corriendo en el core real, mismo resultado que la version escalar.\n");
        return 0;
    } else {
